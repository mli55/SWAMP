...
if __name__ == "__main__":
    ...
    start_freq = 2.0e9
    end_freq   = 5.0e9
    step_freq  = 40e6     # 已将步进改为 40 MHz
    samp_rate  = 500e3
    tx_gain    = 60

    # Instantiate once (reuse the top_block).
    # If you encounter issues, move this into the loop to create a new transmitter each time.
    transmitter = CFRTransmitter(
        start_freq, end_freq, step_freq,
        samp_rate, tx_gain,
        receiver_ip="10.153.27.81",  # <-- Replace with receiver's IP
        receiver_port=9999
    )
