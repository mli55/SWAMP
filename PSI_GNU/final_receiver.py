...
class CFRReceiver(gr.top_block):
    def __init__(
        ...
        samp_rate,
        rx_gain,
        capture_time=1,                 # Capture for 1 second by default
    ...
    def start_measurement(self):
        ...
        Main measurement loop:
          ...
          3) For each connection, sweep from start_freq to end_freq:
             ...
             - Write data
             - Tune to next frequency and send "ready" acknowledgment
                ...
                try:
                    ...
                    with open(self.output_file, "ab") as f:
                        while self.current_freq <= self.end_freq:
                            ...
                            # 8) Prepare for next frequency
                            self.current_freq += self.step_freq

                            # If there's still another frequency to go, tune to it
                            if self.current_freq <= self.end_freq:
                                self.usrp_source.set_center_freq(self.current_freq)
                            else:
                                print("[Receiver] Frequency sweep complete.")

                            # 9) Notify transmitter we are ready for next step
                            try:
...
if __name__ == "__main__":
    ...
    samp_rate  = 500e3
    rx_gain    = 60
    capture_time = 0.125         # capture 0.5 s per frequency
    output_file = "/home/amelia/cfr_data.bin"
