...
void write_to_file(const std::string& srsValue) {
    // Create and open the file inside a function
    std::ofstream outFile("/home/tfei3/example.txt",std::ios::app);
                                        ...
                                        bool                              is_nr)
{
  for (size_t i = 0; i < num_ue; i++) {
    ...
    if (not iszero(mac.ues[i].dl_cqi)) {
      ...
      //write_to_file(std::to_string(int(mac.ues[i].dl_cqi))); // CQI write to file
    } else {
      fmt::print("  {:>3.3}", "n/a");
    }
    fmt::print("   {:>1}", int(mac.ues[i].dl_ri));
    //write_to_file(std::to_string(int(mac.ues[i].dl_ri))); // ri write to file
    float dl_mcs = (is_nr) ? mac.ues[i].dl_mcs : phy[i].dl.mcs;
    if (not isnan(dl_mcs)) {
      fmt::print("   {:>2}", int(dl_mcs));
      //write_to_file(std::to_string(int(dl_mcs))); // MCS write to file
    } else {
      fmt::print("   {:>2}", 0);
    }
    if (mac.ues[i].tx_brate > 0) {
      fmt::print(" {:>6.6}", float_to_eng_string((float)mac.ues[i].tx_brate / (mac.ues[i].nof_tti * 1e-3), 1));
      //write_to_file(float_to_eng_string((float)mac.ues[i].tx_brate / (mac.ues[i].nof_tti * 1e-3), 1)); //tx_rate write to file
    } else {
      fmt::print(" {:>6}", 0);
    }
    ...
    if (mac.ues[i].tx_pkts > 0 && mac.ues[i].tx_errors) {
      ...
      //write_to_file(std::to_string(int((float)100 * mac.ues[i].tx_errors / mac.ues[i].tx_pkts))); //tx_error
    } else {
      fmt::print(" {:>3}%", 0);
    }
    ...
    fmt::print("\n");
    //write_endl(); // go to another line after the last metric
  }
}
