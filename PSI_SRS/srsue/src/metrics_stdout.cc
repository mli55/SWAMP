...
std::string current_output_file_path;
...
std::string generate_timestamped_filename() {
    ...
    char buffer[64];
    std::strftime(buffer, sizeof(buffer), "/home/dell2/example_%Y%m%d_%H%M%S.txt", tm);
    return std::string(buffer);
}

void write_header() {
    // Generate a new filename each time you write a header (i.e., new session)
    current_output_file_path = generate_timestamped_filename();

    std::ofstream outFile(current_output_file_path, std::ios::trunc);
    if (outFile.is_open()) {
        outFile << "RSRP (dB) | MCS | SINR (dB) | DL Brate (bps) | BLER (%)" << std::endl;
        outFile << "--------------------------------------------------------------------------" << std::endl;
        outFile.close();
    }
}

void write_to_file(const std::string& srsValue) {
    if (current_output_file_path.empty()) return;  // Avoid writing if no header was created
    std::ofstream outFile(current_output_file_path, std::ios::app);
    if (outFile.is_open()) {
        outFile << srsValue << " ";
        outFile.close();
    }
...
void write_endl() {
    if (current_output_file_path.empty()) return;
    std::ofstream outFile(current_output_file_path, std::ios::app);
    if (outFile.is_open()) {
        outFile << "\n";
        outFile.close();
...
{
  static bool header_written = false; // Ensure the header is written only once
  if (!header_written) {
      write_header();
      header_written = true;
  }
  ...
  write_to_file(std::to_string(int(phy.ch[r].rsrp))); //rsrp write to file
  fmt::print(" {:>4}", int(phy.ch[r].pathloss));
  //write_to_file(std::to_string(int(phy.ch[r].pathloss))); // pathloss write to file
  fmt::print(" {:>5.5}", float_to_eng_string(phy.sync[r].cfo, 2));
  //write_to_file(float_to_eng_string(phy.sync[r].cfo, 2)); // CFO write to file
  ...
  write_to_file(std::to_string(int(phy.dl[r].mcs))); // write MCS to file
  if (std::isnan(phy.ch[r].sinr) || std::isinf(phy.ch[r].sinr)) {
    fmt::print("  {:>3}", "n/a");
  } else {
    fmt::print("  {:>3}", int(phy.ch[r].sinr));
    write_to_file(std::to_string(int(phy.ch[r].sinr))); // sinr write to file
  }
  fmt::print("  {:>4.1f}", phy.dl[r].fec_iters);

  fmt::print(" {:>6.6}", float_to_eng_string((float)mac[r].rx_brate / (mac[r].nof_tti * 1e-3), 2));
  write_to_file(float_to_eng_string((float)mac[r].rx_brate / (mac[r].nof_tti * 1e-3), 2)); // brate write to file
  if (mac[r].rx_pkts > 0) {
    fmt::print("  {:>3}%", int((float)100 * mac[r].rx_errors / mac[r].rx_pkts));
    write_to_file(std::to_string(int((float)100 * mac[r].rx_errors / mac[r].rx_pkts))); // bler write to file
    write_endl();
  } else {
    fmt::print("  {:>3}%", 0);
  }
